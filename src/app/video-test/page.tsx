...
// Test document for video player
const testVideoDocument = {
  id: 'test-video-1',
...
// Test Video Player component that uses the test endpoint
function TestVideoPlayer({ document }: { document: any }) {
  ...
  useEffect(() => {
    const loadVideo = async () => {
      try {
        console.log('Loading test video for document:', {
          id: document.id,
        ...
        // Use the test endpoint instead of the authenticated endpoint
...
export default function VideoTestPage() {
  ...
  useEffect(() => {
    // Test the video endpoint first
    const testVideoEndpoint = async () => {
      try {
        console.log('Testing video endpoint...')
        const response = await fetch('/api/test/video')
        console.log('Video endpoint response:', {
          status: response.status,
          statusText: response.statusText,
          ok: response.ok,
          headers: Object.fromEntries(response.headers.entries())
        ...
        if (!response.ok) {
          const errorText = await response.text()
          throw new Error(`Video endpoint failed: ${response.status} ${response.statusText} - ${errorText}`)
        }
        ...
        console.error('Video endpoint test failed:', err)
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto mb-4"></div>
          <p className="text-gray-600">Testing video endpoint...</p>
        </div>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <p className="text-red-500 text-lg mb-2">Video Test Failed</p>
  ...
  return (
    <div className="container mx-auto px-8 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-bold gradient-text font-poppins mb-3">Video Player Test</h1>
          <p className="text-slate-600 text-lg">
            Testing video player with test video file
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="p-4 border-b">
            ...
            <p className="text-sm text-slate-500">
              File: {testVideoDocument.originalName} | Type: {testVideoDocument.mimeType}
            </p>
          </div>
          <div className="h-96">
            <TestVideoPlayer document={testVideoDocument} />
          </div>
        </div>
      </div>
    </div>
